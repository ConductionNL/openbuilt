...
- [ ] 1.1 Confirm the OR-side `AuditTrailMapper::getDistinctActorCount(schemaIds, hours)`
      ...
      notes.
- [ ] 1.2 Create `lib/Service/ApplicationInsightsService.php`. Constructor injects
...
- [ ] 1.4 Window-to-hours mapping per REQ-OBAI-004: `7d → 168`, `30d → 720`, `90d → 2160`.
      Centralise the map in a private const on the service.
- [ ] 1.5 Empty schema-set returns four zeros + empty activity (REQ-OBAI-003 last scenario).
      Do not short-circuit to skip OR calls — defensive zeros are simpler.
- [ ] 1.6 PHPDoc on the class and every public method. SPDX header inside the opening
...
- [ ] 2.1 Create `lib/Controller/ApplicationInsightsController.php` extending
      `OCP\AppFramework\Controller`. Constructor injects `ApplicationInsightsService`,
      `IUserSession`, `IRequest`.
- [ ] 2.2 Implement `getInsights(string $appUuid, string $versionUuid): JSONResponse`:
...
- [ ] 2.3 Annotate the method with `#[NoAdminRequired]` (REQ-OBAI-002, REQ-OBAI-007).
      The RBAC gate lives inside the service.
- [ ] 2.4 Add the route entry to `appinfo/routes.php`: verb `GET`, URL
      `/api/applications/{appUuid}/versions/{versionUuid}/insights`, mapping to
      `ApplicationInsightsController::getInsights` (REQ-OBAI-007).
- [ ] 2.5 PHPDoc on the class and every public method. SPDX header inside the opening
...
- [ ] 3.1 Create `src/components/applicationDetail/ApplicationDetailHeader.vue` (single-file
      component). Props: `objectId` (Application UUID from the route param).
- [ ] 3.2 Read `$route.query._version` for the active version slug. If absent, fall
      back to the production version's slug (via the `useApplicationVersion` composable
      from `openbuilt-version-routing`).
- [ ] 3.3 Render the hero strip (REQ-OBADO-001): icon from
      `Application.icon` (per ADR-001), name, description, status badge, caller role
      badge, production version semver.
- [ ] 3.4 Render the version pill strip (REQ-OBADO-002): one pill per version in
...
- [ ] 3.5 Render the window toggle (REQ-OBADO-003): three buttons `7d` / `30d` / `90d`,
      default `7d`. Local component state. Right-align on the same row as the pill strip.
- [ ] 3.6 Create `src/composables/useApplicationInsights.js` — wraps the insights
...
- [ ] 3.7 Render the KPI grid (REQ-OBADO-004): four `CnCard` instances in a
...
- [ ] 3.8 Render the activity graph card (REQ-OBADO-005): use `CnActivityChart` (or
...
- [ ] 3.9 Render the five structural widget cards in a responsive grid below the
...
- [ ] 3.10 Render a "version no longer accessible" banner when the insights endpoint
...
- [ ] 4.1 Create `src/components/applicationDetail/widgets/RegisterWidget.vue`.
      Props: `appSlug`, `versionSlug`, `kpis` (object with `schemaCount`, `objectCount`,
      `filesCount`).
- [ ] 4.2 Render the register name, slug (`openbuilt-{appSlug}-{versionSlug}`), schema
      count, object count, files count.
- [ ] 4.3 "Open in OpenRegister" button — clicking navigates the browser to
...
- [ ] 5.1 Create `src/components/applicationDetail/widgets/SchemasWidget.vue`.
      Props: `appSlug`, `versionSlug`, `schemas` (array of `{ id, name, objectCount, status }`).
- [ ] 5.2 Render each schema as a row with name + object count + status. Row click
      ...
      router config.
- [ ] 5.3 Inline "+ Add schema" header button. On click:
...
- [ ] 6.1 Create `src/components/applicationDetail/widgets/GroupsWidget.vue`.
      Props: `application` (the full Application record so the widget can read
      `permissions.{owners,editors,viewers}`).
- [ ] 6.2 Flatten the three permissions arrays into a single rows array with role
...
- [ ] 6.3 Row click opens the existing permissions editor. The exact route (e.g.
...
- [ ] 7.1 Create `src/components/applicationDetail/widgets/PagesWidget.vue`.
      Props: `appSlug`, `versionSlug`, `pages` (the manifest's `pages[]` array).
- [ ] 7.2 Render each page as a row with id, route, type, title. Row click → router
...
- [ ] 8.1 Create `src/components/applicationDetail/widgets/MenuWidget.vue`.
      Props: `appSlug`, `versionSlug`, `menu` (the manifest's `menu[]` array).
- [ ] 8.2 Render each entry as a row with label, route, order, section. Row click →
...
- [ ] 9.1 In `ApplicationDetailHeader.vue`, render a small Promote affordance (icon
...
- [ ] 9.2 Hide the Promote affordance on the production pill (it is the terminal
      node).
- [ ] 9.3 Click handler:
...
- [ ] 10.1 Open `src/manifest.json`. Locate the `VirtualAppDetail` page entry.
- [ ] 10.2 Add the key `"headerComponent": "ApplicationDetailHeader"` to that entry
      (REQ-OBADO-011).
- [ ] 10.3 Locate the `sidebarTabs` array on the entry. Remove the element whose `id`
...
- [ ] 10.5 Re-parse the JSON to confirm no duplicate keys (per the
      `json-merge-revalidate` memory rule).

## 11. Register ApplicationDetailHeader.vue as a global component

- [ ] 11.1 Locate the component-registration site (usually `src/main.js` or
      `src/index.js`). Register `ApplicationDetailHeader` so the manifest-driven
      `headerComponent` lookup can resolve it by name.
- [ ] 11.2 Confirm the lazy-loading pattern matches the existing convention for
      other manifest-referenced components.

## 12. PHPUnit — ApplicationInsightsService

- [ ] 12.1 Create `tests/Unit/Service/ApplicationInsightsServiceTest.php`. Use
      PHPUnit with `@phpstan-ignore-line` only where unavoidable.
- [ ] 12.2 Tests covering every scenario in `specs/application-insights/spec.md`
...
- [ ] 13.1 Create `tests/Unit/Controller/ApplicationInsightsControllerTest.php`.
- [ ] 13.2 Tests covering REQ-OBAI-001 and REQ-OBAI-006:
...
- [ ] 14.1 Create `tests/e2e/application-detail-overview.spec.ts`. Use the existing
      fixtures from `openbuilt-versioning-model` (`hello-world` Application with
      `development → staging → production` chain).
- [ ] 14.2 Test: page renders all six rows in DOM order (REQ-OBADO-001).
- [ ] 14.3 Test: pill strip renders chain order with production starred
      (REQ-OBADO-002).
...
- [ ] 14.6 Test: window toggle change reloads windowed KPIs only — Object count
...
- [ ] 14.9 Test: Promote affordance renders only on non-terminal pills, hidden on
